...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 17, 2024 at 06:14 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `agregar_pedido` (IN `p_propietario_id` INT, IN `p_cant_tarjetas` INT, IN `p_cost_tarjeta` INT, IN `p_empresa_prov` VARCHAR(40), IN `p_corte_tarjeta` INT, IN `p_porcentaje_descuento` INT)   BEGIN
    ...
    INSERT INTO pedido (propietario_f_id_propietario, cant_tarjetas, cost_tarjeta, fecha_pedido)
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `aumentar_pedidos` ()   BEGIN
    UPDATE pedido SET activo = 0 WHERE activo = 1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminar_pedido` (IN `p_pedido_id` INT)   BEGIN
    -- Eliminar los detalles del pedido
    DELETE FROM detalle_p WHERE pedido_f_num_pedido = p_pedido_id;
    -- Eliminar el pedido
    DELETE FROM pedido WHERE num_pedido = p_pedido_id;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `obtener_pedidos_activos` ()   BEGIN
    ...
    FROM pedido p
    INNER JOIN detalle_p d ON p.num_pedido = d.pedido_f_num_pedido
    WHERE p.activo = 1;
END$$
...
--
-- Table structure for table `detalle_p`
--
...
--
-- Dumping data for table `detalle_p`
--

INSERT INTO `detalle_p` (`empresa_prov`, `corte_tarjeta`, `porcentaje_descuento`, `pedido_f_num_pedido`) VALUES
('Viva', 50, 0, 3),
('Entel', 30, 50, 4),
('Viva', 30, 10, 8),
('Entel', 10, 0, 10),
...
--
DELIMITER $$
CREATE TRIGGER `delete_detalle_p_trigger` AFTER DELETE ON `detalle_p` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `edit_detalle_p_trigger` AFTER UPDATE ON `detalle_p` FOR EACH ROW BEGIN
    INSERT INTO historial (evento, tabla_afectada, id_registro_afectado, fecha)
...
INSERT INTO `pedido` (`num_pedido`, `fecha_pedido`, `propietario_f_id_propietario`, `cant_tarjetas`, `cost_tarjeta`, `activo`) VALUES
...
(25, '2024-06-16', 11, 30, 100, 1);

--
-- Triggers `pedido`
--
DELIMITER $$
CREATE TRIGGER `tr_pedido_delete` AFTER DELETE ON `pedido` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tr_pedido_insert` AFTER INSERT ON `pedido` FOR EACH ROW BEGIN
    INSERT INTO historial (evento, tabla_afectada, id_registro_afectado)
...
INSERT INTO `propietario` (`persona_f_ci_persona`, `id_propietario`) VALUES
(8383883, 11);
...
INSERT INTO `proveedor` (`id_proovedor`, `nombre_empresa`, `correo`) VALUES
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_entel`  AS SELECT `detalle_p`.`empresa_prov` AS `empresa_prov`, `detalle_p`.`corte_tarjeta` AS `corte_tarjeta`, `detalle_p`.`porcentaje_descuento` AS `porcentaje_descuento`, `detalle_p`.`pedido_f_num_pedido` AS `pedido_f_num_pedido` FROM `detalle_p` WHERE `detalle_p`.`empresa_prov` = 'Entel' ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_pedidoscon_descuento`  AS SELECT `detalle_p`.`empresa_prov` AS `empresa_prov`, `detalle_p`.`corte_tarjeta` AS `corte_tarjeta`, `detalle_p`.`porcentaje_descuento` AS `porcentaje_descuento`, `detalle_p`.`pedido_f_num_pedido` AS `pedido_f_num_pedido` FROM `detalle_p` WHERE `detalle_p`.`porcentaje_descuento` > 0 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_tigo`  AS SELECT `detalle_p`.`empresa_prov` AS `empresa_prov`, `detalle_p`.`corte_tarjeta` AS `corte_tarjeta`, `detalle_p`.`porcentaje_descuento` AS `porcentaje_descuento`, `detalle_p`.`pedido_f_num_pedido` AS `pedido_f_num_pedido` FROM `detalle_p` WHERE `detalle_p`.`empresa_prov` = 'Tigo' ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_viva`  AS SELECT `detalle_p`.`empresa_prov` AS `empresa_prov`, `detalle_p`.`corte_tarjeta` AS `corte_tarjeta`, `detalle_p`.`porcentaje_descuento` AS `porcentaje_descuento`, `detalle_p`.`pedido_f_num_pedido` AS `pedido_f_num_pedido` FROM `detalle_p` WHERE `detalle_p`.`empresa_prov` = 'Viva' ;
...
--
ALTER TABLE `pedido`
  MODIFY `num_pedido` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26;
...
--
ALTER TABLE `bd_tk`
  ADD CONSTRAINT `bd_f_tk_detallep_f_fk` FOREIGN KEY (`detallep_f_pedido_f_num_pedido`) REFERENCES `detalle_p` (`pedido_f_num_pedido`),
  ADD CONSTRAINT `bd_f_tk_tarjeta_f_fk` FOREIGN KEY (`tarjeta_f_cod_tarjeta`) REFERENCES `tarjeta` (`cod_tarjeta`);
...
--
-- Constraints for table `detalle_p`
--
ALTER TABLE `detalle_p`
...
ALTER TABLE `dt_fk`
  ...
  ADD CONSTRAINT `dt_f_fk_tarjeta_f_fk` FOREIGN KEY (`tarjeta_f_cod_tarjeta`) REFERENCES `tarjeta` (`cod_tarjeta`);
